    ...
    def get_layer():
        print(
            torch.nn.functional.pad(
                printed_layer, (0, 160 - printed_layer.shape[-1]), "constant", math.nan
            )
        )
...
class personal_trainer:
    ...
    def validate(self, validation, model):
        ...
        for properties in validation:
            ...
            coordinates = (
                properties["coordinates"].to(self.device).float().requires_grad_(True)
            )
                ...
                for i, properties in tqdm.tqdm(
                    ...
                    if self.mtl_loss:
                        ...
                        if self.forces and self.dipole:
                            ...
                            force_loss = (
                                mse(true_forces, forces).sum(dim=(1, 2))
                                / (3.0 * num_atoms)
                            ).mean()
                            dipole_loss = (
                                torch.sum(
                        ...
                        if self.charges:
                            charge_loss = (
                                mse(predicted_charges, true_charges).sum(dim=1)
                                / num_atoms
                            ).mean()
                            # dipole_loss = (torch.sum((mse(predicted_dipoles, true_dipoles))/3.0, dim=1) / num_atoms.sqrt()).mean()
                        ...
                        if self.dipole:
                            raise NotImplementedError(
                                "We currently arent doing dipoles."
                            )
                            dipole_loss = (
                                torch.sum(
                                    (mse(predicted_dipoles, true_dipoles)) / 3.0, dim=1
                    ...
                    AdamW.step()
                    training_writer.add_scalar(
                        "batch_loss", loss, LRscheduler.last_epoch * len(training) + i
                    )
                    training_writer.add_scalar(
                        "learning_rate", learning_rate, LRscheduler.last_epoch
                    )
                self.save_model(nn, AdamW, energy_shifter, latest_pt, LRscheduler)

    def _print_layer(self, nn):
        print_nn = []
        for model in nn:
            new_layers = []
            for i, layer in enumerate(nn[model]):
                if isinstance(layer, torch.nn.Linear):
                    new_layers.append(layer)
                    new_layers.append(PrintLayer())
                else:
                    new_layers.append(layer)
            print_nn.append(torch.nn.Sequential(*new_layers))
        return print_nn

    def model_builder(self, aev_computer, wkdir, checkpoint, printing=False):
        modules = self.setup_nets(aev_computer.aev_length)
        if self.personal:
            ...
            nn.load_state_dict(checkpoint["model"], strict=False)
            if printing:
                nn = torchani.ANIModel(self._print_layer(nn))
            model = torchani.nn.Sequential(aev_computer, nn).to(self.device)
        return model, nn

    def model_loader(self, wkdir, checkpoint, printing=False):
        aev_computer = self.AEV_Computer()
        energy_shifter = self.Energy_Shifter()
        model, nn = self.model_builder(aev_computer, wkdir, checkpoint, printing)
