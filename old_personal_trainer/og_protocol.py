...
class personal_trainer:
    ...
    def datasets_loading(self, energy_shifter):
        if self.ds_path is None:
            ...
            training = torchani.datasets.ANIBatchedDataset(
                self.ds_path, split="training"
            )
            validation = torchani.datasets.ANIBatchedDataset(
                self.ds_path, split="validation"
            )
            training = torch.utils.data.DataLoader(
                training,
                shuffle=True,
    ...
    def pt_setup(self):
        ...
        shutil.copy(
            self.train_file, "{}/trainer.py".format(log, str(date), self.projectlabel)
        )
        if self.personal:
            shutil.copy(
                "models/nets.py",
                "{}/model.py".format(log, str(date), self.projectlabel),
            )
    ...
    def validate(self, validation, model):
        ...
        for properties in validation:
            ...
            if self.charges:
                total_excess_mse += mse_sum(
                    excess_charge, torch.zeros_like(excess_charge)
                ).item()
    ...
    def trainer(self):
        ...
        for _ in range(LRscheduler.last_epoch + 1, self.max_epochs):
            ...
            if LRscheduler.is_better(valrmse["energy_rmse"], LRscheduler.best):
                print(
                    "Saving the model, epoch={}, RMSE = {}".format(
                        (LRscheduler.last_epoch + 1), valrmse["energy_rmse"]
                    )
                )
            ...
            for i, properties in tqdm.tqdm(
                ...
                if self.dipole:
                    dipole_loss = (
                        torch.sum((mse(predicted_dipoles, true_dipoles)) / 3.0, dim=1)
                        / num_atoms.sqrt()
                    ).mean()
