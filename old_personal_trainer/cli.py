...
def main():
    parser = argparse.ArgumentParser(
        prog="cli", description="Command Line Arguments for personal_trainer package."
    )
    ...
    parser_init = subparsers.add_parser(
        "initiate", help="Initiate a project", parents=[common_parser]
    )

    args = parser.parse_args()

    if args.workflow == "initiate":
        project_name = Path("training_center")
        assert (
            not project_name.exists()
        ...
        shutil.copy(
            Path(__file__).parents[1] / "templates/template_model_loader.py",
            project_name,
        )
        shutil.copy(
            Path(__file__).parents[1] / "templates/template_trainer.py", project_name
        )
        shutil.copy(
            Path(__file__).parents[1] / "templates/template_nets.py",
            (project_name / "models").as_posix(),
        )
