...
class ANIModelCharge(torch.nn.ModuleList):
    ...
    def __init__(self, modules, aev_computer):
        ...
        chemical_hardness = torch.tensor([12.84, 10.00, 14.53, 12.16, 8.28, 14.02, 9.35])
        #nmax = (electronegativity**2)/(2*chemical_hardness)                         #electrophilicity
        nmax = (electronegativity/chemical_hardness)#.type(torch.LongTensor)       #nmax
    ...
    def get_coulomb(self, charges, coordinates, species):
        ...
        distance_matrix_mask = self.get_atom_neighbor_mask(
            self.get_atom_mask(species)
        )
...
#Correction function that implements nmax and the networks charge predictions
    ...
    def forward(self, species_coordinates, total_charge=0):
        ...
        return species, molecular_energies, output, output_c, excess_charge, coulomb, correction
